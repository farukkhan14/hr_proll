...
class EmpLeavesController extends Controller {
    ...
    public function actionStatusChangedHistory($id){
        ...
        $data=EmpLeaveStatusHistory::model()->findAll(array('condition'=>$condition), 'id');
        if($data){
            ...
            foreach($data as $d):
                echo "<tr>";
                echo "<td>";
                EmpLeaves::model()->statusColor($d->status);
                echo "</td>";
    ...
    public function actionApprove($id){
        EmpLeaves::model()->updateByPk($id, array('is_approved'=>  EmpLeaves::APPROVED));
        $model=new EmpLeaveStatusHistory;
        $model->emp_leave_id=$id;
        $model->status=EmpLeaves::APPROVED;
    ...
    public function actionDeny($id){
        EmpLeaves::model()->updateByPk($id, array('is_approved'=>  EmpLeaves::DENIED));
        $model=new EmpLeaveStatusHistory;
        $model->emp_leave_id=$id;
        $model->status=EmpLeaves::DENIED;
        $model->status_changed_by=Yii::app()->user->getId();
        $model->status_changed_time= new CDbExpression('NOW()');
        $model->save();
    ...
    public function actionAdmin() {
        $model = new EmpLeaves('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['EmpLeaves']))
            $model->attributes = $_GET['EmpLeaves'];
    ...
    public function loadModel($id) {
        $model = EmpLeaves::model()->findByPk($id);
        if ($model === null)
